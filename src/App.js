...
// ==========================================
// [필독] 디자인 강제 적용 장치 (건드리지 마세요)
// ==========================================
const DesignEnforcer = () => {
  useEffect(() => {
    // 1. Tailwind CSS (디자인 도구) 강제 주입
    if (!document.getElementById('tailwind-script')) {
      const script = document.createElement('script');
      script.id = 'tailwind-script';
      script.src = "https://cdn.tailwindcss.com"; // 디자인 도구 주소
...
// --- Firebase 설정 (반드시 본인 것으로 변경 필요) ---
// 변경하지 않으면 로그인이 안 되거나 오류가 날 수 있습니다.
...
export default function App() {
  ...
  const handleLogout = () => { setCurrentUser(null); setLoginForm({id:'',pw:''}); setModalType(null); };
              ...
              if(slots.length===0) return isInteractive ? (
                <div key={i} className="flex gap-4 items-start group">
                  ...
                  <div className="flex-1 border rounded-xl p-3 bg-gray-50 border-dashed flex justify-between items-center min-h-[60px]">
                    ...
                    <Button size="sm" variant="secondary" icon={PlusCircle} onClick={()=>{setRequestData({...requestData, startTime:t}); setModalType('request_change'); handleRequestAdd(t);}}>추가</Button>
                  </div>
                </div>
              ) : (
                      ...
                      <div key={s.id} className={`border rounded-xl p-3 flex flex-col justify-center min-h-[80px] ${s.status==='confirmed'?'bg-green-50 border-green-200':'bg-blue-50 border-blue-200'}`}>
                        <div className="flex justify-between items-start w-full">
                          ...
                          <div className="flex gap-1">
                            {isInteractive && s.status==='open' && <Button size="sm" variant="danger" icon={XCircle} onClick={()=>{setSelectedSession(s); setRequestData({reason:'', type:'cancel'}); setModalType('request_change');}}>취소</Button>}
                            {currentUser.role==='admin' && <button onClick={()=>deleteSession(s.id)} className="text-red-400 p-1"><Trash2 size={14}/></button>}
                            {currentUser.role==='admin' && s.status==='pending' && <Button size="sm" onClick={()=>{setSelectedSession(s); setModalType('message_preview_confirm');}}>승인</Button>}
  ...
  if(!currentUser) return <><DesignEnforcer/><LoginView/></>;
