...
const generateTimeSlots = () =>
  Array.from({ length: 12 }, (_, i) => `${i + 10}:00`);
  ...
  <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
    <div className="bg-white w-full max-w-md rounded-2xl shadow-xl p-8">
      ...
      <div className="space-y-4">
        ...
        <Button onClick={onLogin} className="w-full py-3 text-lg">
          로그인
        </Button>
      </div>
    </div>
...
const CalendarView = ({
  ...
  const mySessions = isInteractive
    ? sessions.filter(
        (s) => s.taId === currentUser.id && s.date === selectedDateStr
      )
      ...
      <Card className="lg:col-span-1 min-h-[400px]">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold flex items-center gap-2">
            <CalendarIcon size={18} className="text-blue-600" /> 달력
          </h3>
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-lg flex flex-col items-center justify-center relative ${
                  isSel ? 'bg-blue-600 text-white shadow' : 'hover:bg-gray-100'
                }`}
              >
                <span className={`text-sm ${isSel ? 'font-bold' : ''}`}>
                  {d.getDate()}
                </span>
                {has && (
                  <div
            ...
            if (slots.length === 0)
              return isInteractive ? (
                <div key={i} className="flex gap-4 items-start group">
                  <div className="w-16 pt-3 text-right text-sm text-gray-500">
                    {t}
                  </div>
                ...
                <div key={i} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1 border rounded-xl p-3 bg-gray-50 min-h-[60px] flex items-center justify-center text-gray-400 text-xs">
                    일정 없음
                  </div>
                </div>
              );
              ...
              <div key={i} className="flex gap-4 items-start">
                ...
                <div className="flex-1 space-y-2">
                  {slots.map((s) => (
                    <div
                      key={s.id}
                      className={`border rounded-xl p-3 flex flex-col justify-center min-h-[80px] ${
                        s.status === 'confirmed'
...
export default function App() {
  ...
  const handleLogin = () => {
    const u = users.find(
      (u) => u.userId === loginForm.id && u.password === loginForm.password
    );
    if (u) {
      setCurrentUser(u);
      setLoginError('');
      addNotification(`${u.name}님 환영합니다!`);
  ...
  const handleSaveDefaultSchedule = async () => {
    ...
    let count = 0;
    for (
      let d = new Date(batchDateRange.start);
      d <= new Date(batchDateRange.end);
      ...
      if (sched && sched.active) {
        const sH = parseInt(sched.start.split(':')[0]),
          eH = parseInt(sched.end.split(':')[0]);
        for (let h = sH; h < eH; h++) {
          const sT = `${String(h).padStart(2, '0')}:00`,
            eT = `${String(h + 1).padStart(2, '0')}:00`;
          if (
            !sessions.some(
              (s) =>
                s.taId === targetTa.id && s.date === dStr && s.startTime === sT
            )
          ) {
            batch.set(
              doc(
  ...
  const onCancel = (s) => {
    ...
    setRequestData({ reason: '', type: 'cancel' });
    setModalType('request_change');
  };
  const onApprove = (s) => {
    if (!s.classroom) {
      addNotification('강의실을 먼저 선택해주세요', 'error');
      return;
    }
    setSelectedSession(s);
  ...
  const handleRequestCancel = async () => {
    if (!requestData.reason)
      return addNotification('사유를 입력해주세요', 'error');
  ...
  const scheduleRequests = sessions.filter(
    (s) =>
      s.status === 'cancellation_requested' || s.status === 'addition_requested'
  );
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-full max-w-md px-4 space-y-2 pointer-events-none">
        {notifications.map((n) => (
          <div
            key={n.id}
            className={`backdrop-blur text-white px-4 py-3 rounded-lg shadow-xl flex items-center gap-3 justify-center ${
          ...
          <div className="space-y-8">
            ...
            <Card className="border-purple-200 bg-purple-50/30">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <ClipboardList className="text-purple-600" /> 근무 변경 요청
                관리{' '}
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="font-bold">{req.taName}</span>
            ...
            <Card className="bg-blue-50 border-blue-200">
                ...
                {DAYS.map((d) => (
                  ...
                  >
                    <div className="flex justify-between mb-1">
                      ...
                      <input
                        type="checkbox"
                        checked={defaultSchedule[d].active}
                        onChange={() =>
                          setDefaultSchedule({
                            ...defaultSchedule,
                            [d]: {
                              ...defaultSchedule[d],
                              active: !defaultSchedule[d].active,
                            },
                          })
                        }
                      />
                    ...
                    <input
                      ...
                      value={defaultSchedule[d].start}
                      onChange={(e) =>
                        setDefaultSchedule({
                          ...defaultSchedule,
                          [d]: { ...defaultSchedule[d], start: e.target.value },
                        })
                      }
                    />
                    <input
                      ...
                      value={defaultSchedule[d].end}
                      onChange={(e) =>
                        setDefaultSchedule({
                          ...defaultSchedule,
                          [d]: { ...defaultSchedule[d], end: e.target.value },
                        })
                      }
                    />
                  </div>
              ...
              <Button onClick={handleSaveDefaultSchedule} className="w-full">
                설정 저장
              </Button>
            </Card>
                ...
                {pendingBookings.length === 0 ? (
                  ...
                  <div className="grid gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    {pendingBookings.map((s) => (
                      <div
                        key={s.id}
                        className="border-l-4 border-green-500 bg-white shadow-sm rounded-r-lg p-4 flex flex-col gap-2"
                      >
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-bold">{s.studentName}</span>
                            <span className="text-xs text-gray-500">
                              ({s.studentPhone}) → {s.taName}
                            </span>
                          </div>
                          <div className="text-sm text-gray-600">
                        ...
                        <select
                          ...
                          value={s.classroom || ''}
                          onChange={(e) =>
                            updateSession(s.id, { classroom: e.target.value })
                          }
                        >
        ...
        {currentUser.role === 'student' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="col-span-1">
              <h3 className="font-bold mb-4">예약 날짜</h3>
              ...
              <div className="grid grid-cols-7 gap-1">
                {getDaysInMonth(studentDate).map((d, i) => {
                  ...
                  const has = sessions.some(
                    (s) => s.date === dStr && s.status === 'open'
                  );
                  return (
                    <button
                      key={i}
                      onClick={() => setStudentSelectedDateStr(dStr)}
            ...
            <Card className="lg:col-span-2">
              <h3 className="font-bold mb-4">{studentSelectedDateStr} 예약</h3>
              <div className="grid grid-cols-2 gap-4">
                {sessions
                  .filter(
                    (s) =>
                      s.date === studentSelectedDateStr && s.status === 'open'
                  )
                  .map((s) => (
                    <div
                      key={s.id}
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-500">{s.taName}</div>
                    </div>
                  ))}
              </div>
              {studentSelectedSlots.length > 0 && (
                <Button
                  ...
                  onClick={() => setModalType('student_apply')}
                >
                  {studentSelectedSlots.length}건 예약 신청
                </Button>
              )}
            </Card>
          </div>
      ...
      >
        ...
        />
        <Button onClick={handleRequestCancel} className="w-full">
          요청 전송
        </Button>
      </Modal>
      <Modal
      ...
      >
        {applicationItems.map((item, i) => (
          <div key={i} className="border rounded p-3 mb-2 bg-gray-50">
            <input
              ...
              value={item.subject}
              onChange={(e) => {
                const n = [...applicationItems];
                n[i].subject = e.target.value;
                setApplicationItems(n);
              }}
            />
            <div className="flex gap-2">
              <input
        ...
        <Button className="w-full" onClick={submitStudentApplication}>
          신청 완료
        </Button>
      </Modal>
      <Modal
      ...
      >
        <textarea
          ...
          onChange={(e) =>
            setFeedbackData({ ...feedbackData, clinicContent: e.target.value })
          }
        />
        <textarea
          className="w-full border rounded p-2 mb-2 h-20"
        ...
        <textarea
          ...
          onChange={(e) =>
            setFeedbackData({ ...feedbackData, improvement: e.target.value })
          }
        />
      ...
      >
        ...
        <div className="flex gap-2 mb-4">
          ...
          />
          <Button onClick={() => createUser({ ...newUser, role: manageTab })}>
            추가
          </Button>
        </div>
        <div className="max-h-[300px] overflow-auto">
          {users
            .filter((u) => u.role === manageTab)
            .map((u) => (
              <div
                key={u.id}
              ...
              >
                <div>
                  ...
                  <span className="text-gray-500 text-xs">({u.userId})</span>
                </div>
                <button
      ...
      <Modal
        ...
        title="문자 발송"
      >
        <div className="bg-gray-50 p-4 rounded mb-4 whitespace-pre-wrap text-sm">
          {selectedSession && TEMPLATES.confirmParent(selectedSession)}
        </div>
        <Button
          className="w-full"
          onClick={async () => {
            await updateSession(selectedSession.id, { status: 'confirmed' });
            setModalType(null);
            addNotification('발송 완료');
          }}
        >
          전송 및 확정
        </Button>
      ...
      >
        ...
        <Button
          className="w-full mt-4"
          onClick={async () => {
            await updateSession(selectedSession.id, { feedbackStatus: 'sent' });
            setModalType(null);
            addNotification('발송 완료');
          }}
        >
