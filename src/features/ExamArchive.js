...
import { 
  ...
  FileQuestion, BookOpen, PenTool, ExternalLink, ShieldCheck 
} from 'lucide-react';
import { collection, query, where, getDocs, doc, runTransaction, updateDoc, serverTimestamp, limit } from 'firebase/firestore';
import { db } from '../firebase';
import { Button, Card, Modal, Badge } from '../components/UI';

const APP_ID = 'imperial-clinic-v1';

// [CTO 수정] 요구사항에 맞춘 5가지 자료 종류 정의
const FILE_TYPES = [
...
const ExamArchive = ({ currentUser }) => {
    // 필터 상태: 명시적 검색을 위해 로컬에서만 관리
    const [filters, setFilters] = useState({
        schoolType: '', district: '', schoolName: '', year: '', semester: '', term: '', subject: ''
    });
    const [exams, setExams] = useState([]);
    const [loading, setLoading] = useState(false);
    
    // 모달 관리: 특정 시험의 특정 파일에 대한 작업 수행
    ...
    // --- 2. 동시성 제어 및 파일별 중복 방지 (Transaction) ---
    const handleClaimTask = async (exam, fileKey) => {
        ...
        try {
            // [비용/보안] 병렬 작업 중 다른 조교와 데이터 덮어쓰기 충돌이 나지 않도록 트랜잭션으로 원자성 보장
    ...
    return (
        <div className="space-y-6 w-full animate-in fade-in">
            <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-gray-500 font-medium">학교별 세부 자료 현황 관리</span>
            </div>
                    ...
                    <table className="w-full text-left text-sm min-w-[1000px]">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {exams.length === 0 && !loading && (
                                <tr><td colSpan="2" className="text-center py-12 text-gray-400 bg-white">상단에서 검색 조건을 설정해주세요.</td></tr>
                            )}
                            {exams.map(exam => (
