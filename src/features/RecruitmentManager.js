...
export default function RecruitmentManager() {
    ...
    const sendRecruitmentSMS = async (type, applicantData, scheduleData = null) => {
        ...
        try {
            await addDoc(collection(db, `artifacts/${APP_ID}/public/data/sms_outbox`), {
                phoneNumber: cleanPhone, message: message, status: 'pending', type: 'hr_recruitment', studentName: applicantData.name, createdAt: serverTimestamp()
            });
            alert('안내 문자가 발송 대기열에 등록되었습니다.');
        } catch (error) {
    ...
    const handleAddApplicant = async () => {
        ...
        try {
            await addDoc(collection(db, `artifacts/${APP_ID}/public/data/recruitment`), {
                ...form, status: 'applied', createdAt: serverTimestamp(), updatedAt: serverTimestamp()
            });
    ...
    const handleScheduleInterview = async () => {
        ...
        try {
            await setDoc(doc(db, `artifacts/${APP_ID}/public/data/recruitment`, selectedApplicant.id), {
                status: 'scheduled', interviewDate: scheduleForm.interviewDate, interviewTime: scheduleForm.interviewTime, updatedAt: serverTimestamp()
            }, { merge: true });
    ...
    const updateStatus = async (applicant, newStatus, smsType) => {
        const confirmMsg = smsType ? `상태를 변경하고 지원자에게 자동 안내 문자를 발송하시겠습니까?` : `상태를 변경하시겠습니까?`;
        if (!window.confirm(confirmMsg)) return;

        try {
            await setDoc(doc(db, `artifacts/${APP_ID}/public/data/recruitment`, applicant.id), { status: newStatus, updatedAt: serverTimestamp() }, { merge: true });
            if (smsType) await sendRecruitmentSMS(smsType, applicant);
    ...
    const getStatusBadge = (status) => {
        switch(status) {
            case 'applied': return <Badge className="bg-slate-100 text-slate-600">서류 심사 중</Badge>;
            case 'scheduled': return <Badge className="bg-blue-100 text-blue-700">면접 대기</Badge>;
            case 'passed': return <Badge className="bg-emerald-100 text-emerald-700">면접 합격 (범죄조회 중)</Badge>;
            case 'bg_checked': return <Badge className="bg-purple-100 text-purple-700">조회 완료 (계약 대기)</Badge>;
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-20 animate-in fade-in">
                                        ...
                                        <div className="flex flex-wrap items-center gap-4 text-sm font-bold text-slate-500">
                                            ...
                                            {app.status === 'scheduled' && (
                                                <span className="flex items-center gap-1 text-blue-600 bg-blue-50 px-2 py-1 rounded-md">
                                                    <CalendarIcon size={14}/> {app.interviewDate} {app.interviewTime} 면접 예정
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                        ...
                                        {app.status === 'scheduled' && (
                                            <>
                                                <Button onClick={() => updateStatus(app, 'passed', 'passed')} className="bg-emerald-600 hover:bg-emerald-700 text-xs py-1.5 px-3">
                                                    <CheckCircle size={14} className="mr-1 inline"/> 합격 통보 (범죄조회 요청)
                                                </Button>
                                                <Button onClick={() => updateStatus(app, 'rejected', 'rejected')} variant="outline" className="text-xs py-1.5 px-3 border-rose-200 text-rose-600 hover:bg-rose-50">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalMode === 'add' ? '신규 지원자 접수' : '면접 일정 조율 및 안내'}>
                <div className="space-y-4 p-2">
                    {modalMode === 'add' ? (
                        ...
                        <>
                            <div className="bg-blue-50 text-blue-700 p-3 rounded-xl text-xs font-bold border border-blue-200 mb-4 leading-relaxed">
