...
export default function SchoolStrategy({ currentUser }) {
  ...
  const [activeTerm, setActiveTerm] = useState("1학기 중간고사");
  ...
  const handleSearchSubmit = async () => { 
      ...
      try {
          ...
          if (filterInput.grade !== '전체' || filterInput.exam !== '전체') {
              results = results.filter(r => {
                  const rGrade = String(r.grade || '1학년');
                  const rSemester = String(r.semester || '1학기');
                  ...
                  if (filterInput.grade !== '전체') { gradeMatch = rTerm.startsWith(filterInput.grade); }
                  if (filterInput.exam !== '전체') { examMatch = rTerm.includes(filterInput.exam); }
  ...
  const handleOpenForm = (existingReport = null) => {
      ...
      if (existingReport) {
          ...
          setFormData({ 
              type: 'individual', year: defaultYear, school: '', term: '1학년 1학기 중간고사', subject: initSubject, standardCode: '', 
          ...
          reader.onload = async () => {
              try {
                  ...
                  const result = await analyzeFn({
                      ...
                      mimeType: file.type,
                      year: formData.year,
                      grade: formData.term ? formData.term.split(' ')[0] : '1학년',
                      subject: formData.subject || '수학'
                  });
                  ...
                  setFormData(prev => ({
                      ...
                      gradeCuts: {
                          grade1: aiData.cutoffs?.top10 || prev.gradeCuts?.grade1 || '',
                          grade2: aiData.cutoffs?.top34 || prev.gradeCuts?.grade2 || '',
                          grade3: aiData.cutoffs?.top66 || prev.gradeCuts?.grade3 || '',
                      },
                      questions: aiData.questions ? aiData.questions.map((q, idx) => {
                          const idi = q.idi || {};
                          const srcFam = idi.sourceFamiliarity || 1;
                          const logTrans = idi.transformationLogic || 1;
                          const conInt = idi.conceptualIntegration || 1;
                          const calcComp = idi.calculationComplexity || 1;
                          const logDep = idi.logicalDepth || 1;
                          const totalIdi = srcFam + logTrans + conInt + calcComp + logDep;
                          
                          let diff = '하';
                          if (totalIdi >= 20) diff = '최상';
                          else if (totalIdi >= 15) diff = '상';
                          else if (totalIdi >= 10) diff = '중';

                          return {
                              qNum: q.number ? `${q.number}번` : (q.qNum || `${idx + 1}번`),
                              score: q.score || '',
                              unit: q.unit || '',
                              tags: q.tags || '',
                              idiSource: srcFam,
                              ...
                              idiTotal: totalIdi,
                              diff: diff,
                              source: q.source || '자체 분석',
                              analysis: q.comment || q.analysis || ''
                          };
                      }) : prev.questions
                  }));

                  alert("✅ AI 분석이 완료되어 데이터가 자동 입력되었습니다.\n내용을 확인하시고, 우측 상단의 [저장하기]를 눌러주세요.");
              } catch (err) {
  ...
  const handleSaveReport = async () => {
    ...
    if(!formData.term) return alert("학년 및 시험 학기를 정확히 선택해주세요.");
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <select className="w-full border p-2.5 rounded-lg text-sm outline-none font-bold focus:ring-2 focus:ring-blue-200" value={formData.term} onChange={e => {
                  ...
                  {['1학년', '2학년', '3학년'].flatMap(g => 
                      ['1학기 중간고사', '1학기 기말고사', '2학기 중간고사', '2학기 기말고사'].map(t => 
                          <option key={`${g} ${t}`} value={`${g} ${t}`}>{`${g} ${t}`}</option>
                      )
                  )}
              </select>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm animate-in zoom-in-95 duration-300">
                <div>
                   ...
                   <p className="text-xs text-indigo-700 mt-1 break-keep">시험지 PDF를 업로드하면 <b>총평, 예상 등급컷, 문항별 배점, 단원, IDI 지표</b>를 AI가 빈칸에 자동으로 입력해 줍니다.</p>
                </div>
                <div className="shrink-0 w-full md:w-auto">
                   <input type="file" ref={fileInputRef} className="hidden" accept=".pdf" onChange={handleAiPdfUpload} />
                   <Button onClick={() => fileInputRef.current?.click()} disabled={isAiAnalyzing} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all">
                       {isAiAnalyzing ? <Loader className="animate-spin w-4 h-4"/> : <UploadCloud className="w-4 h-4"/>}
                       {isAiAnalyzing ? 'AI가 분석 중입니다 (약 30초)' : 'PDF 업로드하여 자동 채우기'}
                   </Button>
                </div>
              </div>
              ...
              <div className="mb-4">
                 ...
                 <div className="flex gap-2">
                    <input type="text" className="w-1/3 border p-2 text-sm rounded outline-none text-center focus:border-indigo-400" placeholder="1등급 (예: 92)" value={formData.gradeCuts?.grade1 || ''} onChange={e => setFormData({...formData, gradeCuts: { ...formData.gradeCuts, grade1: e.target.value }})}/>
                    <input type="text" className="w-1/3 border p-2 text-sm rounded outline-none text-center focus:border-indigo-400" placeholder="2등급 (예: 85)" value={formData.gradeCuts?.grade2 || ''} onChange={e => setFormData({...formData, gradeCuts: { ...formData.gradeCuts, grade2: e.target.value }})}/>
                    <input type="text" className="w-1/3 border p-2 text-sm rounded outline-none text-center focus:border-indigo-400" placeholder="3등급 (예: 78)" value={formData.gradeCuts?.grade3 || ''} onChange={e => setFormData({...formData, gradeCuts: { ...formData.gradeCuts, grade3: e.target.value }})}/>
                 </div>
              </div>
              ...
              <div><label className="block text-xs font-bold mb-1 text-indigo-700">시험 총평</label><textarea className="w-full border p-2 text-sm rounded min-h-[100px] outline-none focus:border-indigo-400" value={formData.review} onChange={e => setFormData({...formData, review: e.target.value})}/></div>
              ...
              <h3 className="text-lg font-bold border-b pb-2 pt-4 flex justify-between items-center text-indigo-900">
                문항별 상세 분석
                ...
                {formData.questions?.map((q, idx) => (
                  <div key={idx} className="bg-gray-50 border p-3 md:p-4 rounded-xl relative shadow-sm">
                    ...
                    <div className="grid grid-cols-4 md:grid-cols-6 gap-2 mb-3">
                      <div><label className="text-[10px] text-gray-500 font-bold">번호 (예: 1번)</label><input type="text" placeholder="1번" className="w-full border p-1.5 text-sm rounded outline-none font-bold" value={q.qNum} onChange={e=>handleArrayChange('questions', idx, 'qNum', e.target.value)}/></div>
                      <div><label className="text-[10px] text-indigo-500 font-bold">배점</label><input type="number" onWheel={(e) => e.target.blur()} className="w-full border border-indigo-200 focus:border-indigo-500 p-1.5 text-sm rounded outline-none font-bold" value={q.score} onChange={e=>handleArrayChange('questions', idx, 'score', e.target.value)}/></div>
                      ...
                      <div><label className="text-[10px] text-gray-500 font-bold">난이도 (자동)</label><input type="text" className="w-full border p-1.5 text-sm bg-gray-200 text-gray-600 rounded font-bold text-center" readOnly value={`${q.diff || '하'} (${q.idiTotal || 5})`} /></div>
                    ...
                    <div className="grid grid-cols-5 gap-1 md:gap-2 mb-3 bg-indigo-50 p-2 rounded-lg border border-indigo-100">
                      <div><label className="text-[9px] md:text-[10px] text-indigo-700 font-bold break-keep">출처친숙도(1-5)</label><input type="number" onWheel={(e) => e.target.blur()} min="1" max="5" className="w-full border border-indigo-200 p-1 text-sm rounded text-center outline-none font-bold text-indigo-900" value={q.idiSource || 1} onChange={e=>handleIdiChange(idx, 'idiSource', e.target.value)}/></div>
                      <div><label className="text-[9px] md:text-[10px] text-indigo-700 font-bold break-keep">변형로직(1-5)</label><input type="number" onWheel={(e) => e.target.blur()} min="1" max="5" className="w-full border border-indigo-200 p-1 text-sm rounded text-center outline-none font-bold text-indigo-900" value={q.idiLogic || 1} onChange={e=>handleIdiChange(idx, 'idiLogic', e.target.value)}/></div>
                      <div><label className="text-[9px] md:text-[10px] text-indigo-700 font-bold break-keep">개념결합(1-5)</label><input type="number" onWheel={(e) => e.target.blur()} min="1" max="5" className="w-full border border-indigo-200 p-1 text-sm rounded text-center outline-none font-bold text-indigo-900" value={q.idiConcept || 1} onChange={e=>handleIdiChange(idx, 'idiConcept', e.target.value)}/></div>
                      <div><label className="text-[9px] md:text-[10px] text-indigo-700 font-bold break-keep">연산복잡(1-5)</label><input type="number" onWheel={(e) => e.target.blur()} min="1" max="5" className="w-full border border-indigo-200 p-1 text-sm rounded text-center outline-none font-bold text-indigo-900" value={q.idiCalc || 1} onChange={e=>handleIdiChange(idx, 'idiCalc', e.target.value)}/></div>
                      <div><label className="text-[9px] md:text-[10px] text-indigo-700 font-bold break-keep">논리전개(1-5)</label><input type="number" onWheel={(e) => e.target.blur()} min="1" max="5" className="w-full border border-indigo-200 p-1 text-sm rounded text-center outline-none font-bold text-indigo-900" value={q.idiProg || 1} onChange={e=>handleIdiChange(idx, 'idiProg', e.target.value)}/></div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
                      ...
                      <div><label className="text-[10px] text-indigo-600 font-bold">분석 코멘트</label><input type="text" className="w-full border focus:border-indigo-400 p-1.5 text-sm rounded outline-none" value={q.analysis} onChange={e=>handleArrayChange('questions', idx, 'analysis', e.target.value)}/></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="p-4 md:p-8">
            {!report.type || (!report.review && (!report.questions || report.questions.length === 0)) ? (
                <div className="flex flex-col items-center justify-center py-16 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    <span className="text-gray-500 font-bold mb-2">분석 데이터가 아직 작성되지 않았습니다.</span>
                    {isStaff && (
              ...
              <div className="p-4 md:p-6 bg-yellow-50/30 border-t border-yellow-200 animate-fade-in">
                ...
                {report.files && Object.values(report.files).some(f => f && f.url) && (
                  <div className="mb-4 p-3 bg-white border border-yellow-300 rounded-lg flex flex-col gap-2 shadow-sm">
                    ...
                    {['studentWork', 'examPaper', 'quickAnswer', 'solution'].map(key => { // analysis 제외
                      const fileData = report.files[key];
                      if (!fileData || !fileData.url) return null;
                      const labels = { studentWork: '학생풀이(원본)', examPaper: '시험지', quickAnswer: '빠른답지', solution: '해설' };
                      return (
