/* [서비스 가치(Service Value)] 스마트 아날로그 Voca 클라이언트 포털 v3.1
   🚀 업데이트 8 (학부모 리포트 내장): 시험 완료 상태일 때 AI 취약 어휘 리포트를 제공.
   🚀 업데이트 9 (학습 이력 투명화): 최근 10회차의 시험 응시 로그를 O(1) 병렬 쿼리로 호출.
   🚀 업데이트 10 (AI 4-Core 대시보드): 4코어 밸런스를 보여주는 대시보드 개편.
   🚀 업데이트 11 (Zero-Query 100% Sync): 백엔드(vocaEngine)의 queueType과 프론트엔드 대시보드를 완벽히 동기화. 학부모 클릭 시 추가 DB 조회 없이 메모리 캐시를 활용하여 비용을 0원으로 통제. */
...
const StudentVocaDaily = ({ currentUser }) => {
  ...
  // 🚀 [CTO 패치] Zero-Query 아키텍처: DB를 긁지 않고, vocaEngine.js가 만든 QueueType을 그대로 정제하여 사용.
  const matrixDataSource = useMemo(() => {
      if (sessionInfo.status === 'ready') return wordsList || [];
      if (studentStats?.parentReport?.vulnerableWords) {
          // completed 또는 pending 상태일 경우 가장 최근에 저장된 리포트 데이터 활용
          return studentStats.parentReport.vulnerableWords.map(w => ({
              ...w,
              queueType: w.type, 
              word: w.word,
              meanings: [{ koreanMeaning: w.meaning }],
              incorrectCount: w.type === '만성 오답' ? 3 : (w.type === '오답' ? 1 : 0) // 시각적 표기를 위한 mock data
          }));
      }
      return [];
  }, [sessionInfo.status, wordsList, studentStats]);

  const countNew = matrixDataSource.filter(w => ['신규', '딥 스캔', '의심 스캔'].includes(w.queueType)).length;
  const countReview = matrixDataSource.filter(w => w.queueType === '복습').length;
  const countPassive = matrixDataSource.filter(w => w.queueType === '패시브').length;
  const countWrong = matrixDataSource.filter(w => ['오답', '만성 오답'].includes(w.queueType)).length;
  ...
  const handleMatrixClick = async (type) => {
      ...
      // UX를 위해 DB를 불러오는 듯한 0.3초의 인공 지연(UX Delay) 부여
      await new Promise(resolve => setTimeout(resolve, 300));

      let filtered = [];
      if (type === 'new') filtered = matrixDataSource.filter(w => ['신규', '딥 스캔', '의심 스캔'].includes(w.queueType));
      if (type === 'review') filtered = matrixDataSource.filter(w => w.queueType === '복습');
      if (type === 'passive') filtered = matrixDataSource.filter(w => w.queueType === '패시브');
      if (type === 'chronic') {
          // 일반 오답과 만성 오답을 묶어서 보여주되, 정렬하여 만성 오답이 상단에 오도록 처리
          filtered = matrixDataSource.filter(w => ['오답', '만성 오답'].includes(w.queueType));
          filtered.sort((a, b) => (b.queueType === '만성 오답' ? 1 : 0) - (a.queueType === '만성 오답' ? 1 : 0));
      }

      setMatrixData(filtered);
  ...
  const handlePrint = (type = 'wordbook') => {
    ...
    dataToPrint.forEach((w, i) => {
      ...
      const wrongCountHtml = type === 'vulnerable' && w.queueType === '만성 오답' 
        ? `<div class="wrong-badge">🚨 만성 오답 타격 대상</div>` 
        : '';
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8 animate-in fade-in pb-20">
      ...
      {/* 🚀 탭 2: AI 4-Core 어휘 밸런스 매트릭스 (100% 동기화 대시보드) */}
      {activeTab === 'matrix' && (
          <div className="animate-in fade-in slide-in-from-bottom-4">
              <div className="flex flex-col mb-6 bg-gradient-to-br from-indigo-50 to-blue-50 p-6 sm:p-8 rounded-[32px] border border-indigo-100 shadow-sm">
                  ...
                  {/* O(1) 정적 렌더링 카드 - DB 읽기 0회. 엔진이 만든 배열 길이를 그대로 출력하여 100% 매칭 */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      >
                          <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-lg font-black text-blue-500">{countNew}단어</span>
                          </div>
                          ...
                          <div className="flex justify-between items-center text-blue-600 font-bold text-sm mt-auto">
                      ...
                      >
                          <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-lg font-black text-emerald-500">{countReview}단어</span>
                          </div>
                          ...
                          <div className="flex justify-between items-center text-emerald-600 font-bold text-sm mt-auto">
                      ...
                      >
                          <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-lg font-black text-amber-500">{countPassive}단어</span>
                          </div>
                          ...
                          <div className="flex justify-between items-center text-amber-600 font-bold text-sm mt-auto">
                      ...
                      >
                          <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-lg font-black text-rose-500">{countWrong}단어</span>
                          </div>
                          <h4 className="font-black text-lg text-slate-800">오답 추적 (Wrong)</h4>
                          <p className="text-xs font-bold text-slate-500 mt-1 mb-3 break-keep">어제 발생한 일반 오답 및 3회 이상 틀린 만성 오답입니다.</p>
                          <div className="flex justify-between items-center text-rose-600 font-bold text-sm mt-auto">
              ...
              {/* 🚀 On-Demand Fetching 결과 영역 (Zero-Query) */}
              {expandedMatrix && (
                          ...
                          <>
                              ...
                              <div className={`p-5 rounded-2xl border mb-6 flex gap-3 ${
                                  ...
                                  <div className="font-bold text-sm leading-relaxed">
                                      ...
                                      {expandedMatrix === 'chronic' && "이전에 틀렸던 오답들을 집중 추적합니다. 특히 3회 이상 반복해서 틀린 '만성 오답'의 경우 내일 시험부터 예문 빈칸 추론이나 다의어 매칭 등 강제 변형 형태로 최우선 배정됩니다."}
                                  </div>
                              </div>
                                  ...
                                  <div className="text-center py-10">
                                      ...
                                      <p className="font-bold text-slate-500">
                                          {sessionInfo.status === 'pending' 
                                            ? "오늘자 학습 데이터가 준비되지 않았거나, 대기 중인 단어가 없습니다." 
                                            : "현재 해당 카테고리에 대기 중인 단어가 없거나 시스템에 의해 완벽히 제어되고 있습니다."}
                                      </p>
                                  </div>
                              ) : (
                                  <table className="w-full text-left border-collapse">
                                      ...
                                      <tbody>
                                          {matrixData.map((word, idx) => {
                                              ...
                                              const isChronic = word.queueType === '만성 오답';
                                              
                                              return (
                                                  <tr key={idx} className={`border-b border-slate-100 transition-colors ${isChronic ? 'bg-rose-50/50 hover:bg-rose-50' : 'hover:bg-slate-50'}`}>
                                                      ...
                                                      <td className="p-3 align-middle">
                                                          ...
                                                          <div className={`text-xs font-black px-2 py-0.5 rounded-md inline-block mt-1 ${isChronic ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500'}`}>
                                                              {isChronic ? `🚨 만성 오답 타격 대상 (변형 출제 예정)` : (expandedMatrix === 'chronic' ? `✅ 일반 오답 (1차 재시험 대기)` : `✅ 모니터링 및 주기 검증 중`)}
                                                          </div>
                                                      </td>
                                                  </tr>
                                              );
