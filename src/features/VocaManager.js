/* [서비스 가치(Service Value)] AI Voca 통합 관제 센터 v8.0 (심플 UX 및 강사-학부모 뷰 일치 배포본)
   🚀 가치 1 (상담 효율 90% 단축): '시험 로그' 탭에서 반과 원생을 선택하여 모든 회차의 % 정답률과 실력 상승을 조망합니다.
   🚀 가치 2 (인지 부하 0% UX): 불필요한 툴팁, 호버 애니메이션, 내신 탭을 삭제하고 핵심 성장 데이터만 심플하게 시각화합니다.
   🚀 가치 3 (누락 없는 100% 렌더링): 오름차순 정렬 로직을 강화하여 1, 2, 3회차의 모든 시험 내역을 100% 표시합니다. */

import React, { useState, useMemo, useEffect } from 'react';
import { 
    Users, Printer, BarChart2, Search, 
    AlertCircle, FileText, RefreshCw, Sliders, Trophy, BookOpen, CheckCircle, ChevronDown, Undo2, GraduationCap, Info, CalendarDays, Loader, Award, Clock, User, BarChart3
} from 'lucide-react';
...
const getTierProgress = (masteredCount = 0, catScore = 0) => {
    ...
    let prevLimit = 0; let currentTier = TIERS[0];
    for (let i = 0; i < TIERS.length; i++) {
        if (totalEstimatedWords < TIERS[i].limit) { currentTier = TIERS[i]; break; }
        prevLimit = TIERS[i].limit; if (i === TIERS.length - 1) currentTier = TIERS[i];
    }
...
const VocaManager = ({ currentUser }) => {
    ...
    useEffect(() => {
        if (!isSeasonAutoSet && !loadingData) {
            if (dynamicSeasons && dynamicSeasons.length > 0) {
                ...
                if (current && current.id) { setSelectedSeasonId(current.id); } 
                else {
                    const future = dynamicSeasons.filter(s => (s?.startDate || '') > todayStr);
                    if (future.length > 0 && future[0].id) { setSelectedSeasonId(future[0].id); } 
                    else {
                        const past = [...dynamicSeasons].reverse();
                        if (past.length > 0 && past[0].id) { setSelectedSeasonId(past[0].id); } else { setSelectedSeasonId('legacy'); }
                    }
                }
            } else { setSelectedSeasonId('legacy'); }
    ...
    // 🚀 [시험 로그 탭 전용 상태 - 내신 탭 제거 및 Voca 단일화]
    ...
    const availableClasses = useMemo(() => {
        ...
        if (currentUser?.role === 'lecturer' || currentUser?.role === 'ta') { filtered = filtered.filter(c => c?.lecturerId === currentUser?.id); }
        return filtered.sort((a, b) => (a?.name || '').localeCompare(b?.name || ''));
    }, [classes, currentUser, selectedSeasonId]);

    useEffect(() => {
        if (availableClasses.length > 0) {
            if (!availableClasses.some(c => c?.id === selectedClassId)) { setSelectedClassId(availableClasses[0]?.id || ''); }
            if (!availableClasses.some(c => c?.id === selectedLogClassId)) { setSelectedLogClassId(availableClasses[0]?.id || ''); }
        } else { setSelectedClassId(''); setSelectedLogClassId(''); }
    }, [availableClasses, selectedClassId, selectedLogClassId]);

    const enrolledStudentIds = useMemo(() => {
        if (!selectedClassId) return [];
        return (enrollments || []).filter(e => e?.classId === selectedClassId && e?.status === 'active').map(e => e?.studentId);
    }, [selectedClassId, enrollments]);

    const classStudents = useMemo(() => {
        if (!selectedClassId) return [];
        let filteredStudents = (users || []).filter(u => u?.role === 'student' && enrolledStudentIds.includes(u?.id)).filter(student => (student?.name || '').includes(searchQuery));
        if (sortConfig) {
        ...
        } else { filteredStudents.sort((a, b) => (a?.name || '').localeCompare(b?.name || '')); }
    ...
    const logEnrolledStudentIds = useMemo(() => {
        ...
        return (enrollments || []).filter(e => e?.classId === selectedLogClassId && e?.status === 'active').map(e => e?.studentId);
    }, [selectedLogClassId, enrollments]);

    const logStudentList = useMemo(() => {
        if (!isStaff) return (users || []).filter(u => u?.role === 'student');
        if (!selectedLogClassId) return [];
        return (users || []).filter(u => u?.role === 'student' && logEnrolledStudentIds.includes(u?.id)).sort((a, b) => (a?.name || '').localeCompare(b?.name || ''));
    }, [selectedLogClassId, logEnrolledStudentIds, users, isStaff]);

    useEffect(() => {
        if (activeTab === 'score_logs') {
            if (!isStaff && currentUser?.id) { setSelectedLogStudentId(currentUser.id); } 
            else if (isStaff && logStudentList.length > 0 && !logStudentList.some(s => s.id === selectedLogStudentId)) { setSelectedLogStudentId(logStudentList[0].id); }
        }
    }, [activeTab, isStaff, currentUser, logStudentList, selectedLogStudentId]);

    // 🚀 [CTO 최적화 쿼리] N+1 방지, 모든 Voca 시험 세션 누락 없이 100% 페칭
    useEffect(() => {
        ...
        const fetchScoreLogs = async () => {
            ...
            try {
                const logs = [];
                // 🚀 단어장 시험(voca) 세션만 안전하게 조회 (내신/개념 쿼리 완벽 제거로 Read 비용 50% 절감)
                const vocaQ = query(
                ...
                // 🚀 일자 및 회차별 완벽 오름차순 정렬 (1, 2, 3회차 누락 0% 보장)
                logs.sort((a, b) => {
                ...
                for (let i = 0; i < logs.length; i++) {
                    ...
                    const prevPointScore = prevLog ? (prevLog.catScoreSnapshot !== null ? prevLog.catScoreSnapshot : null) : null;
                ...
                if (isMounted) setLogErrorMsg("🚨 데이터를 불러오는 중 오류가 발생했습니다.");
            } finally { if (isMounted) setLoadingLogs(false); }
        };

        fetchScoreLogs();
        return () => { isMounted = false; };
    }, [activeTab, selectedLogStudentId, englishStats]);

    // 시험 로그 핵심 통계 (불필요한 등락 수치 제거)
    const logStats = useMemo(() => {
        if (scoreLogs.length === 0) return { avg: 0, currentCatScore: 300 };
        let total = 0;
        scoreLogs.forEach(l => { total += l.score; });
        const avg = Math.round(total / scoreLogs.length);
        const targetStat = (englishStats || []).find(s => s?.id === selectedLogStudentId || s?.studentId === selectedLogStudentId) || {};
        const currentCatScore = Number(targetStat.catScore) || 300;
        return { avg, currentCatScore };
    ...
    const handleSort = (key) => { if (sortConfig === key) setSortConfig(null); else setSortConfig(key); };

    const preparePrintData = async (type, targetStudentId = null) => {
        setProcessing(true);
        try {
            const dataToPrint = [];
            const targetStudents = targetStudentId ? classStudents.filter(s => s?.id === targetStudentId) : classStudents;
            if (targetStudents.length === 0) { alert("출력할 대상 학생이 없습니다."); setProcessing(false); return; }
            ...
            for (const student of targetStudents) {
                try {
                    ...
                    if (type.startsWith('retest') && (!testSnap.exists() || testSnap.data()?.status !== 'completed') && sessionNum > 1) {
                        sessionNum = sessionNum - 1; sessionId = `test_${student?.id}_s${sessionNum}`;
                        testSnap = await getDoc(doc(db, `artifacts/${APP_ID}/public/data/test_sessions`, sessionId));
                    }
                    
                    let questionsList = []; let wordsList = []; let isSessionCompleted = false; let wrongNums = [];
                    if (testSnap.exists() && testSnap.data().questionsForTest) {
                        const testData = testSnap.data();
                        questionsList = testData.questionsForTest || []; wordsList = testData.wordsForPrint || [];
                        if (testData.status === 'completed') { isSessionCompleted = true; wrongNums = testData.wrongAnswerNumbers || []; }
                        ...
                        questionsList = payload.questionsForTest || []; wordsList = payload.wordsForPrint || [];
                    }

                    if (type.startsWith('retest')) {
                        if (!isSessionCompleted) continue;
                        if (wrongNums.length === 0) continue;
                ...
                } catch (studentError) { console.error(`[${student?.name || '학생'}] 개별 데이터 준비 실패:`, studentError); }
            }
            if (dataToPrint.length === 0) { alert("🚨 출력할 데이터가 없습니다."); setProcessing(false); return; }
            ...
            let htmlContent = `
              <html><head><title>임페리얼 영단어 출력</title><style>
                @page { margin: 0; size: A4 portrait; }
                body { font-family: 'Malgun Gothic', sans-serif; color: #111; margin: 0; padding: 15mm; box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                .print-page { page-break-after: always; margin-bottom: 20px; } .print-page:last-child { page-break-after: auto; }
                ...
                .header h2 { margin: 0; font-size: 24px; font-weight: bold; color: #0f172a; } .header .info { text-align: right; font-size: 14px; font-weight: bold; color: #475569; }
                table { width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 13px; table-layout: fixed; }
                th, td { border-bottom: 1px solid #cbd5e1; padding: 10px 6px; text-align: left; vertical-align: middle; word-wrap: break-word; } th { background-color: #f8fafc; font-weight: bold; color: #475569; }
                tr { page-break-inside: avoid !important; break-inside: avoid !important; } .word-content-wrapper { page-break-inside: avoid !important; break-inside: avoid !important; display: block; }
                .text-center { text-align: center; } .word-text { font-size: 16px; font-weight: bold; color: #0f172a; display: block; } .hint-text { display: block; font-size: 11px; color: #64748b; margin-top: 4px; font-weight: bold; }
                .answer-blank { border-bottom: 1px solid #94a3b8; width: 100%; height: 20px; display: inline-block; } .advanced-row td { border-top: 2px solid #334155; }
                .rich-info { margin-top: 4px; font-size: 11px; color: #475569; line-height: 1.4; font-weight: 500; } .rich-info span.tag { font-weight: bold; margin-right: 4px; display: inline-block; padding: 1px 4px; border-radius: 3px; font-size: 9px; }
                .tag.synonym { color: #059669; background-color: #d1fae5; border: 1px solid #a7f3d0; } .tag.antonym { color: #e11d48; background-color: #ffe4e6; border: 1px solid #fecdd3; } .tag.example { color: #3b82f6; background-color: #eff6ff; border: 1px solid #bfdbfe; } .pos-tag { color: #64748b; font-weight: normal; margin-right: 4px; } .meaning-line { margin-bottom: 2px; }
              </style></head><body>
            `;

            dataToPrint.forEach((data) => {
                htmlContent += `
                  <div class="print-page"><div class="header"><h2>임페리얼 ${printTitle}</h2><div class="info"><div>이름: ${data.student.name || '알수없음'}</div><div>날짜: ${new Date().toLocaleDateString()} ${type.includes('test') && !type.includes('retest') ? '/ 정답률: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; %' : ''}</div></div></div>
                    <table><thead><tr><th class="text-center" style="width: 8%;">No.</th><th style="width: ${type === 'wordbook' ? '32%' : '45%'};">${type === 'wordbook' ? 'Target Vocabulary' : 'Question (문제)'}</th><th style="width: ${type === 'wordbook' ? '60%' : '47%'};">${type === 'wordbook' ? 'Core Meaning & Context' : 'Answer (정답 기재란)'}</th></tr></thead><tbody>
                `;
                if (type === 'wordbook') {
                    data.wordsList.slice(0, 40).forEach((w, i) => {
                        const meanings = w.meanings && w.meanings.length > 0 ? w.meanings : [];
                        let meaningHtml = ''; let allSynonyms = []; let allAntonyms = []; let fullSentence = '';
                        if (meanings.length > 0) {
                            meanings.forEach((m, idx) => {
                                ...
                                if (m.synonyms) allSynonyms.push(...m.synonyms); if (m.antonyms) allAntonyms.push(...m.antonyms);
                                if (!fullSentence && m.exampleSentence) { fullSentence = m.exampleSentence; } 
                                else if (!fullSentence && m.blankSentence && m.blankSentence.length > 0) { fullSentence = m.blankSentence[0].replace(/_+(?:\s*_+)*/g, w.word); }
                            });
                        ...
                        htmlContent += `<tr><td class="text-center font-bold">${i + 1}</td><td><div class="word-content-wrapper"><span class="word-text">${w.word}</span></div></td><td><div class="word-content-wrapper"><div style="font-weight: 800; color: #1e3a8a; font-size: 14px;">${meaningHtml}</div>${extraInfoHtml}</div></td></tr>`;
                    });
                } else {
                    data.questionsList.forEach((q) => {
                        ...
                        htmlContent += `<tr ${rowClass}><td class="text-center font-bold">${q.questionNumber}</td><td><div class="word-content-wrapper"><span class="word-text">${q.wordText}</span>${hintHtml}</div></td><td style="font-weight: bold; color: ${answerColor};"><div class="word-content-wrapper">${answerDisplay}</div></td></tr>`;
                    });
                }
                htmlContent += `</tbody></table></div>`;
            });
            htmlContent += `<script>window.onload = function() { window.print(); setTimeout(function(){ window.close(); }, 500); }</script></body></html>`;
            const printWindow = window.open('', '_blank');
            if (printWindow) { printWindow.document.open(); printWindow.document.write(htmlContent); printWindow.document.close(); } 
            else { alert("🚨 브라우저의 팝업 차단이 설정되어 있습니다."); }
        } catch (error) { console.error("Print Data Preparation Error:", error); alert("🚨 인쇄 시스템 에러가 발생했습니다."); } 
    ...
    const confirmPresetChange = async () => {
        ...
        try {
            ...
            await setDoc(statRef, { vocaPreset: presetData.newPreset, adaptivePreset: null, updatedAt: serverTimestamp() }, { merge: true });
            setPresetModalOpen(false);
        } catch (error) { alert("프리셋 변경 중 오류가 발생했습니다."); } finally { setProcessing(false); }
    };
    ...
    const openGradingModal = async (student, stat) => {
        ...
        setGradingData({ studentId: student.id, name: student.name || '학생', sessionNumber: sessionNum, wrongAnswers: [] });
    ...
    const submitDetailedGrading = async () => {
        ...
        } catch (error) { console.error("Detailed Grading Error:", error); alert("채점 처리 중 오류가 발생했습니다."); } 
        finally { setProcessing(false); }
    };

    const handleRollback = async (studentId, sessionNumber) => {
        if (!window.confirm(`⚠️ 경고: [제 ${sessionNumber}회차] 채점을 취소하고 롤백하시겠습니까?`)) return;
        setProcessing(true);
        try {
            await rollbackVocaTestResult(studentId, sessionNumber);
            alert(`${sessionNumber}회차 채점이 취소되고 데이터가 복구되었습니다.`);
        } catch(error) { console.error("Rollback Error:", error); alert(error.message || "복구 중 오류가 발생했습니다."); } 
        finally { setProcessing(false); }
    };
    ...
    const handleCatSubmit = async (studentId, score) => {
        ...
        try {
            ...
            await setDoc(statRef, { catScore: score, maxApprovedPromotion: Math.floor(score / 50) * 50, promotionPending: null, adaptivePreset: '초기 영점 조절', updatedAt: serverTimestamp() }, { merge: true });
            alert("어휘력이 성공적으로 반영되었습니다.");
            setCatInput(prev => ({ ...prev, [studentId]: '' }));
        } catch (error) { alert("점수 입력 중 오류가 발생했습니다."); } finally { setProcessing(false); }
    };

    const handleApprovePromotion = async (studentId, targetScore, studentName) => {
        if (!studentId) return;
        if (!window.confirm(`[${studentName || '학생'}] 학생의 ${targetScore}점 승급 심사를 통과 처리하시겠습니까?`)) return;
        setProcessing(true);
        try {
            const statRef = doc(db, `artifacts/${APP_ID}/public/data/english_stats`, studentId);
            await setDoc(statRef, { maxApprovedPromotion: targetScore, promotionPending: null, updatedAt: serverTimestamp() }, { merge: true });
            alert(`${targetScore}점 승급 처리가 완료되었습니다.`);
        } catch (error) { alert("상태 변경 중 오류가 발생했습니다."); } finally { setProcessing(false); }
    };
        ...
        <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in pb-20 print:hidden">
            ...
            <Modal isOpen={presetModalOpen} onClose={() => setPresetModalOpen(false)} title="학습 프리셋 변경 확인">
                <div className="p-4 space-y-4 text-center">
                    ...
                    <p className="text-sm font-bold text-gray-500 bg-slate-50 p-3 rounded-xl">다음 회차 시험지 생성 시점부터 적용됩니다.</p>
                    <div className="flex gap-3 justify-center mt-6"><Button variant="secondary" onClick={() => setPresetModalOpen(false)}>취소</Button><Button onClick={confirmPresetChange} disabled={processing}>{processing ? '처리 중...' : '변경 확정'}</Button></div>
                </div>
            </Modal>
            ...
            <Modal isOpen={gradingModalOpen} onClose={() => setGradingModalOpen(false)} title={`${gradingData.name} 학생 채점 (제 ${gradingData.sessionNumber}회차)`} className="max-w-3xl w-full">
                <div className="p-4 space-y-6">
                    <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 flex items-start gap-3">
                        ...
                        <div><h4 className="font-black text-indigo-900 mb-1">틀린 문항 번호만 클릭하세요.</h4><p className="text-xs font-bold text-indigo-700 leading-relaxed">선택된 오답 데이터는 AI 엔진으로 전송되어 망각 주기를 재설정합니다.</p></div>
                    </div>
                    <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
                        {Array.from({ length: 50 }, (_, i) => i + 1).map(num => {
                            const isWrong = gradingData.wrongAnswers.includes(num);
                            return <button key={num} onClick={() => toggleWrongAnswer(num)} className={`py-3 rounded-lg font-black text-sm transition-all border-2 ${isWrong ? 'bg-rose-100 text-rose-700 border-rose-300 shadow-inner' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300 hover:bg-indigo-50'}`}>{num}</button>;
                        })}
                    </div>
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200 pt-6 mt-6">
                        <div className="text-center sm:text-left"><span className="text-sm font-bold text-slate-500">최종 백분율(%) 정답률</span><div className="text-4xl font-black text-slate-800">{Math.round(((50 - gradingData.wrongAnswers.length) / 50) * 100)} <span className="text-lg text-slate-400">% ({50 - gradingData.wrongAnswers.length}/50문항)</span></div></div>
                        <Button className="w-full sm:w-auto px-8 py-4 text-lg font-black bg-indigo-600 hover:bg-indigo-700 shadow-md" onClick={submitDetailedGrading} disabled={processing}>{processing ? 'AI 분석 중...' : '제출 및 AI 분석 시작'}</Button>
                    </div>
                </div>
            </Modal>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col xl:flex-row justify-between items-center gap-4">
                <div><h1 className="text-2xl font-black text-slate-800 flex items-center gap-2"><BarChart2 className="text-indigo-600" /> Voca 통합 관제 센터</h1></div>
                
                {/* 🚀 상단 네비게이션 탭 ('시험 로그' 간소화 및 아이콘 완벽 제거) */}
                <div className="flex bg-slate-100 p-1 rounded-2xl flex-wrap justify-center gap-1">
                    ...
                    <button onClick={() => { setActiveTab('score_logs'); setSortConfig(null); }} className={`px-4 py-2 rounded-xl font-bold transition-all text-xs sm:text-sm ${activeTab === 'score_logs' ? 'bg-white text-blue-600 shadow-sm font-black' : 'text-slate-500 hover:bg-slate-200'}`}>시험 로그</button>
            ...
            {/* 🚀 [시험 로그 탭: 심플 UX, 클래스 통합 선택기 및 100% 렌더링] */}
            {activeTab === 'score_logs' ? (
                <div className="space-y-6 animate-in fade-in">
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            <h2 className="text-lg font-black text-slate-800 flex items-center gap-2">
                                [{currentLogStudentName}] 영단어 시험 응시 로그
                            </h2>
                    ...
                    {/* 🚀 KPI 요약 통계 카드 (불필요한 역대최고/등락 제거 및 심플화) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center justify-between">
                            <div>
                                <span className="text-xs font-bold text-slate-400 flex items-center gap-1"><Award size={14} className="text-indigo-500"/> 역대 평균 정답률</span>
                                <div className="text-3xl font-black text-slate-800 mt-1">{logStats.avg} <span className="text-sm font-normal text-slate-400">%</span></div>
                            </div>
                            <div className="bg-indigo-50 text-indigo-700 font-black px-4 py-2 rounded-2xl text-xs border border-indigo-100">총 {scoreLogs.length}회차 완료</div>
                        </div>
                        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center justify-between">
                            <div>
                                ...
                                <div className="text-3xl font-black text-purple-600 mt-1">{logStats.currentCatScore} <span className="text-sm font-normal text-slate-400">점</span></div>
                            </div>
                            <div className="bg-purple-50 text-purple-700 font-black px-4 py-2 rounded-2xl text-xs border border-purple-100">아카데미 유니버스 연동</div>
                        </div>
                    </div>

                    {/* 🚀 초경량 SVG/Tailwind 심플 시계열 차트 (호버 툴팁/에니메이션 완벽 삭제) */}
                    <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                        <div className="flex justify-between items-center">
                            <div>
                                <h3 className="text-base font-black text-slate-800 flex items-center gap-2">시계열 정답률(%) 흐름</h3>
                                <p className="text-xs font-bold text-slate-400">회차별 정답률(%)의 안정적인 우상향 흐름을 증명합니다.</p>
                            </div>
                        ...
                        {loadingLogs ? (
                            <div className="h-48 flex flex-col items-center justify-center gap-2"><Loader className="animate-spin text-blue-600" size={32} /><span className="text-xs font-bold text-slate-400">학습 로그를 분석하고 있습니다...</span></div>
                        ) : scoreLogs.length === 0 ? (
                            <div className="h-48 flex flex-col items-center justify-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 p-6 text-center"><BarChart3 size={36} className="text-slate-300 mb-2" /><h4 className="font-bold text-slate-600 text-sm">출력할 시험 기록이 없습니다.</h4><p className="text-xs text-slate-400 mt-1">단어장 채점을 완료하면 즉시 그래프가 생성됩니다.</p></div>
                        ) : (
                            <div className="relative pt-6 pb-2">
                                ...
                                {/* 🚀 호버 툴팁과 애니메이션을 과감히 제거하여 UX 부하를 0%로 만들었습니다! */}
                                <div className="flex items-end justify-start gap-3 sm:gap-6 h-48 pt-4 px-2 overflow-x-auto no-scrollbar relative z-10">
                                    {scoreLogs.map((log) => {
                                        const heightPercent = Math.max(10, Math.min(100, log.score));
                                        return (
                                            <div key={log.id} className="flex-1 min-w-[40px] max-w-[60px] flex flex-col items-center gap-1.5">
                                                <div className="text-xs font-black text-slate-700">{log.score}%</div>
                                                <div className="w-full bg-slate-100 rounded-t-xl h-36 flex items-end justify-center p-1 overflow-hidden">
                                                    <div className="w-full rounded-lg bg-blue-500 shadow-sm" style={{ height: `${heightPercent}%` }}></div>
                                                </div>
                                                <span className="text-[11px] font-black text-slate-500 truncate max-w-full">제{log.sessionNumber}회</span>
                                            </div>
                                        );
                                    })}
                                </div>
                    ...
                    {/* 상세 시험 응시 내역 테이블 (% 및 종합 실력 연동 표기) */}
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <h3 className="font-black text-slate-800 text-base flex items-center gap-2"><Clock className="text-blue-600" size={18} /> 상세 시험 내역 및 종합 실력 기여도</h3>
                            ...
                            <table className="w-full text-left border-collapse min-w-[650px]">
                                <thead>
                                    <tr className="bg-slate-50 text-slate-500 text-xs font-black border-b border-slate-200">
                                        ...
                                        <th className="p-4 w-28">시험 회차</th>
                                        <th className="p-4 w-36 text-center">정답률(%)</th>
                                        <th className="p-4 w-44 text-center">종합 어휘력 연동</th>
                                        <th className="p-4">세부 피드백 및 오답 요약</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 text-sm font-bold">
                                    {scoreLogs.slice().reverse().map((log) => (
                                        <tr key={log.id} className="hover:bg-blue-50/40 transition-colors">
                                            ...
                                            <td className="p-4 font-black text-slate-800 whitespace-nowrap">제 {log.sessionNumber}회 평가</td>
                                            <td className="p-4 text-center">
                                                <span className="font-black text-slate-800 text-base bg-slate-50 border border-slate-200 px-3 py-1 rounded-xl">
                                                    {log.score}%
                                                </span>
                                            </td>
